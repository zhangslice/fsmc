...
#define MCUFREQMAX             144000000L
#define LATENCYFREQ            21200000L
/**
* @}
*/
...
u32 exTimeOutCounter;


int SetandCheckCLK(u32 CLKmode, u32 MaxdelayCounter, u32 validcounter)
{
    ...
    if ((CLKmode == RCC_CR_HSION) || (CLKmode == RCC_CR_HSEON) || (CLKmode == RCC_CR_PLLON))
    {
        CLK_ready = CLKmode << 1;
    }
    else
    ...
    while (1)
    {
        ...
        if (ClkSrcStatus == 0)
        {
            ...
            {
                TimeOutCounter++;
                if (TimeOutCounter > MaxdelayCounter)
                {
                    ClkSrcStatus = 3;
                    break;
                }
        ...
        else if (ClkSrcStatus == 1)
        {
            if ((RCC->CR & CLK_ready) != 0)
            {
                GetCLKCounter++;
                if (GetCLKCounter > validcounter)
                {
                    ClkSrcStatus = 2;
                    break;
                }
    ...
    exTimeOutCounter = TimeOutCounter;
    if (ClkSrcStatus == 2)
    {
        return 0x81;
    }
...
void SetSysClockToXX_Define(void)
{
    ...
    exTimeOutCounter = 0;
    if (0x81 == SetandCheckCLK(RCC_CR_HSEON, 0xA000, 1000))
    {
        ClkSrcStatus = 1;
    }
    else
    ...
    if (SystemCoreClock > MCUFREQMAX)
        SystemCoreClock = MCUFREQMAX;
    ...
    if (ClkSrcStatus == (uint32_t)0x01)
    {
        ...
        temp = (SystemCoreClock) / (LATENCYFREQ);
        FLASH->ACR |= temp;
        ...
        else if (SystemCoreClock < 90000000)
        {
            ...
            RCC->CFGR |= (uint32_t)RCC_CFGR_HPRE_DIV1;
            /* PCLK2 = HCLK/2 */
            RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV1;//RCC_CFGR_PPRE2_DIV2;
            /* PCLK1 = HCLK/4 */
            RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE1_DIV2;//RCC_CFGR_PPRE1_DIV4
        }
        ...
        if (SystemCoreClock >= 96000000)
        {
            ...
            while (!(PWR->CSR & (1 << 16)));
            exTimeOutCounter = 0;
            if (0x81 == SetandCheckCLK(RCC_CR_PLLON, 0x1A000, 10))
            {
                ClkSrcStatus = 1;
            }
            else
        ...
        {
            exTimeOutCounter = 0;
            if (0x81 == SetandCheckCLK(RCC_CR_PLLON, 0x1A000, 1000))
            {
                ClkSrcStatus = 1;
            }
            else
